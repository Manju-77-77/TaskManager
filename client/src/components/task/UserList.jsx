...
const UserList = ({ setTeam, team }) => {
  const { allUsers, user } = useSelector((state) => state.user); // Get current user as well
  ...
  useEffect(() => {
    if (team?.length < 1 && filteredUsers?.length > 0) {
  ...
  const handleChange = (selected) => {
    ...
    setTeam(selected); // Store entire user object instead of just IDs
  };
